...
-s
    Site type (v4,v3,tsse,dtfm) REQUIRED

-l
    Site url minus http://www

-k
    Site passkey

-h
    Site secret key (U232 V3/V4)
...
until [[ $siteuservar == carryon ]]; do
echo -n "Enter site type (v4/v3/tsse/dtfm): "
read sitetype
echo -n "Choose site login name: "
read siteuser
echo -n "Confirm site login name '"$siteuser"' (Yes/No)"`tput setaf 3``tput bold`"[YES]: "`tput sgr0`
read yno
case $yno in
        [yY] | [yY][Ee][Ss] | "")
                echo -n "Please enter site login password: "`tput setaf 0``tput setab 0`
                read sitepass
                echo -n `tput sgr0`"Please retype site login password: "`tput setaf 0``tput setab 0`
                read sitepass2
                tput sgr0
                case $sitepass2 in
                        $sitepass )
...
echo -n "Enter site url without the http:// or www: "
read siteurl
echo -n "Enter site passkey: "
read sitepasskey
if [[ $sitetype = "v4" ]]; then
    echo -n "Enter site secret key: "
    read sitesecretkey
fi
if [[ $sitetype = "v3" ]]; then
    echo -n "Enter site secret key: "
...
if [[ -z "$usernamevar" ]] || [[ -z "$passvar" ]] || [[ -z "$sitetype" ]]; then
...
until [[ $sitevar == yes ]]; do
    case $sitetype in
        "v4" )
            files="http://gitlab.open-scene.net/upload-bots/v4-upload-bot.git"
            sitevar=yes
            ;;
        "v3" )
            files="http://gitlab.open-scene.net/upload-bots/v3-upload-bot.git"
            sitevar=yes
            ;;
        "tsse" )
            files="http://gitlab.open-scene.net/upload-bots/tsse-upload-bot.git"
            sitevar=yes
            ;;
        "dtfm" )
            files="http://gitlab.open-scene.net/upload-bots/dtfm-upload-bot.git"
            sitevar=yes
            ;;
    esac
done
...
cp /usr/local/bin/rtorrent /usr/local/bin/whocares
chmod +x /usr/local/bin/whocares
...
sed -i 's/<username>/'$usernamevar'/' /var/www/rutorrent/conf/users/seeding/config.php
...
sed 's/<username>/'$usernamevar'/' /root/flizbox/.rtorrentseed.rc > /home/$usernamevar/.rtorrentseed.rc
...
mkdir -p /home/$usernamevar/rtorrent2_watch
mkdir -p /home/$usernamevar/rtorrent2/.session
...
sed 's/<username>/'$usernamevar'/' /root/flizbox/check-seeding > /home/$usernamevar/scripts/check-seeding
chmod +x /home/$usernamevar/scripts/check-rt
chmod +x /home/$usernamevar/scripts/check-seeding
...
cd /home/$usernamevar
git clone $files upload
sed -i 's/<username>/'$usernamevar'/' /home/$usernamevar/upload/upload.php
sed -i 's/<username>/'$usernamevar'/' /home/$usernamevar/upload/rtorrent.php
sed -i 's/<username>/'$usernamevar'/' /home/$usernamevar/upload/allupload.php
sed -i 's/<siteurl>/'$siteurl'/' /home/$usernamevar/upload/upload.php
sed -i 's/<loginname>/'$siteuser'/' /home/$usernamevar/upload/upload.php
sed -i 's/<password>/'$sitepass'/' /home/$usernamevar/upload/upload.php
sed -i 's/<passkey>/'$sitepasskey'/' /home/$usernamevar/upload/upload.php
if [[ $sitetype = "v4" ]]; then
    sed -i 's/<secretkey>/'$sitesecretkey'/' /home/$usernamevar/upload/upload.php
fi
if [[ $sitetype = "v3" ]]; then
    sed -i 's/<secretkey>/'$sitesecretkey'/' /home/$usernamevar/upload/upload.php
fi
if [[ $sitetype = "tsse" ]]; then
    regex="https?:\/\/"$siteurl"\/download\.php"
    sed -i 's/<regex>/'$regex'/' /home/$usernamevar/upload/upload.php
fi

chown -R $usernamevar:$usernamevar /home/$usernamevar
chmod 777 /home/$usernamevar/upload/rtorrent.php /home/$usernamevar/upload/upload.php
...
echo "@reboot /home/"$usernamevar"/scripts/check-seeding >> /dev/null 2>&1" >> tempcron
echo "*/3 * * * * /home/"$usernamevar"/scripts/check-seeding >> /dev/null 2>&1" >> tempcron
